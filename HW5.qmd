...
1.  cross validation is to check the performance and generalization of the random forest model. Although they already reduce overfitting, cross validation make a more accurate guess of how the model will perform on unseen data

2.  Bagging is an ensemble technique that builds decision tress on different bootstrapped samples of an original dataset. each tree is created independently and the results are averages creating a more stable and accurate prediction.
...
4.  The interaction term allow for the model to check situation where one predictor depends on the value of another., It enables the model to check for more complex relationships between variables.

5.  Data is split into training and test sets to evaluate the model's ability to generalize. The model is trained on a portion of a set then used to see if that training holds up on unseen data. This prevents over fitting, and increases accuracy.
